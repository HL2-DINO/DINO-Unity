...
public class ResearchModeController : MonoBehaviour
{
    ...
    private void ResearchModeSetup(string toolsetString)
    {
        ...
        try
        {
            ...
            ConsoleDebugTextMesh.text = "Successful RMDLL Launch";
        }
        catch (Exception ex)
        {
            ConsoleDebugTextMesh.text = ex.Message;
            ConsoleDebugTextMesh.text += '\n' + ex.StackTrace;
        }
...
#   if ENABLE_WINMD_SUPPORT
    ...
    bool SetupLocator()
    {
        ...
        var unityCoordinateSystem = Microsoft.MixedReality.OpenXR.PerceptionInterop.GetSceneCoordinateSystem(UnityEngine.Pose.identity) as Windows.Perception.Spatial.SpatialCoordinateSystem;
